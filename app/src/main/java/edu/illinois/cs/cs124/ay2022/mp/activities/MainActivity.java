...
public final class MainActivity extends AppCompatActivity
 ...
 private void updateShownPlaces(final List<Place> showPlaces) {
   ...
   for (Place place : showPlaces) {
     ...
     if (place.getDescription().toLowerCase().contains("restaurant")
         || place.getDescription().toLowerCase().contains("cafe")
         || place.getDescription().toLowerCase().contains("bar")
         || place.getDescription().toLowerCase().contains("grill")) {
       marker.setTextIcon("R");
     } else if (place.getDescription().toLowerCase().contains("theater")
         || place.getDescription().toLowerCase().contains("theatre")
         || place.getDescription().toLowerCase().contains("movies")) {
       marker.setTextIcon("T");
     }  else if (place.getDescription().toLowerCase().contains("illini")
         || place.getDescription().toLowerCase().contains("university of illinois")) {
       marker.setTextIcon("UIUC");
     }

     //start ty
     /*
     Drawable markerIcon = ContextCompat.getDrawable(context, R.drawable.marker_default);
     ColorFilter filter = new LightingColorFilter(Color.RED, 1);
     markerIcon.setColorFilter(filter);
     marker.setIcon(markerIcon);
     mapView.getOverlays().add(marker);
     public class MarkerHueSetter extends Marker {
       public void setMarkerHue(Marker marker, float hueSet) {
         if (hueSet < 0 || hueSet > 360) {
           throw new IllegalArgumentException();
         }
         if (hueSet >= 0 && hueSet < 120) {
           // red
         } else if (hueSet <= 120 && hueSet < 240) {
           // green
         } else {
           // blue
         }
 ...
 @Override
 public boolean longPressHelper(final GeoPoint a) {
   Log.d(TAG, "longPress");
   Intent launchAddFavoritePlace = new Intent(this, AddPlaceActivity.class);
   launchAddFavoritePlace.putExtra("latitude", String.valueOf(a.getLatitude()));
   launchAddFavoritePlace.putExtra("longitude", String.valueOf(a.getLongitude()));
   startActivity(launchAddFavoritePlace);
   return false;
 }
 @Override
 public boolean onQueryTextChange(final String newText) {
   List<Place> newList = Place.search(allPlaces, newText);
   if (newList.size() == 0) {
     updateShownPlaces(allPlaces);
   } else {
     updateShownPlaces(newList);
   }
   return true;
 }
}
